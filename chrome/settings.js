...
async function setRedirectState(element, state = true, changeStorage = false) {
    if(element) {
        if (state) {
            ...
            if (changeStorage) {
                try {
                    await chrome.storage.local.set({ redirect: state });
...
async function sendGetRedirectState() {
    return new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({ action: "getRedirect" }).then((response) => {
...
async function sendSetRedirectState(state) {
    return new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({ action: "setRedirect", redirect: state }).then((response) => {
            if (response.success) {
